  ...
  return (
    ...
    >
      <nav className="container mx-auto px-4 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-5 text-cream/90">
            ...
            <button
              onClick={() => {
                if (user) {
                  // If admin, maybe link to dashboard? Or simple logout dropdown
                  // For now, let's just log out or show name
                  if (user.role === 'admin') {
                    navigate('/admin');
                  } else {
                    // Simple logout for now, or could open a profile modal
                    logout();
                  }
                } else {
                  setIsAuthOpen(true);
            ...
            >
              <User className={cn("h-5 w-5", user ? "text-gold fill-gold/20" : "")} />
            </button>
        ...
        <div
          className={cn(
            "absolute left-0 right-0 top-full bg-[#FAFAFA] border-t border-brown-deep/10 shadow-luxury transition-all duration-500 overflow-y-auto",
            hoveredCategory === "Shop" ? "max-h-[80vh] opacity-100 visible" : "max-h-0 opacity-0 invisible"
          )}

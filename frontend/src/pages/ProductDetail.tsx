...
import { Heart, Share2, ShoppingBag, Shield, RefreshCw, Truck, ChevronLeft, ChevronRight, ZoomIn, Check, Star, X, Ruler, Coins, Gem } from "lucide-react";
...
const ProductDetail = () => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        const mappedProduct = {
          ...
          images: data.images.length > 0 ? data.images.map((img: string) => img.startsWith("http") ? img : `${API_BASE_URL}/${img}`) : [],
          rating: 4.8, // Default
          reviews: 124, // Default
          ...
          details: {
            ...
            diamond: {
              ...
              brand: "Miracle",
              quality: data.diamondDetails[0]?.quality || "EF,VVS" // Use actual data
            },
            colorStones: {
          ...
          // Use categoryDetails.name if available (from our backend fix), else try to use category ID if it fails (fallback)
          categoryName: data.categoryDetails?.name || data.category // Store name for logic
        };
        ...
        if (data.diamondDetails?.[0]?.quality) {
          const dq = data.diamondDetails[0].quality;
          // Map backend format to frontend strict types if needed, or just use if matches
          if (dq === "EF-IF-VVS1" || dq === "EF,IF,VVS1") setDiamondQuality("EF,IF-VVS1");
          else setDiamondQuality("EF,VVS");
        }
        if (data.colorStoneDetails?.[0]?.quality) {
          // Ensure precise matching
          const sq = data.colorStoneDetails[0].quality;
          if (sq === "Natural Gems") setSelectedStoneQuality("Natural");
          else if (sq === "Semi Precious") setSelectedStoneQuality("Semi-Precious");
          else if (sq === "Synthetic") setSelectedStoneQuality("Synthetic");
        }
  ...
  useEffect(() => {
    ...
    const calculatePrice = async () => {
      try {
        // Construct payload with current selections
        const payload = {
          goldDetails: {
            ...rawData.goldDetails,
            metalPurity: metalType.toLowerCase(), // e.g. "22kt"
            metalColor: metalColor
          },
          diamondDetails: rawData.diamondDetails.map((d: any) => ({
            ...d,
            quality: diamondQuality // Apply global selection
          })),
          colorStoneDetails: rawData.colorStoneDetails.map((c: any) => ({
            ...c,
            quality: selectedStoneQuality // Apply global selection
          })),
          // CRITICAL: Pass the product's specific pricing parameters (VA, MC)
          pricingDetails: rawData.pricingDetails
        };

        const response = await fetch(`${API_BASE_URL}/api/v1/products/calculate-price`, {
          method: 'POST',
        ...
        if (response.ok) {
          const breakup = await response.json();
          setPricing({
            goldRate: breakup.goldRate,
            goldValue: breakup.goldPrice,
            diamondRate: 0,
            diamondValue: breakup.diamondPrice,
            stoneValue: breakup.stonePrice,
            makingCharges: breakup.makingCharges,
            wastage: breakup.wastage,
            certificationCost: breakup.certificationCost || 0,
            tax: breakup.gst,
            total: breakup.grandTotal,
            diamondBreakup: breakup.diamondBreakup || [],
            stoneBreakup: breakup.stoneBreakup || [],
            netWeight: breakup.netWeight
          });
        }
      } catch (error) {
    ...
    calculatePrice();
  }, [metalType, metalColor, diamondQuality, selectedStoneQuality, product, rawData]);
  ...
  const handleAddToCart = () => {
    if (product) {
      addToCart({
        id: product.id,
        name: product.name,
        price: `₹${Math.round(pricing.total - pricing.tax).toLocaleString()}`,
          ...
          <div className="space-y-4">
            ...
            <div className="relative aspect-square bg-card rounded-2xl overflow-hidden group">
              ...
              <div className="absolute left-4 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                <DiamondArrow direction="left" onClick={prevImage} className="w-10 h-10" iconClassName="w-5 h-5" />
              </div>
              <div className="absolute right-4 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                <DiamondArrow direction="right" onClick={nextImage} className="w-10 h-10" iconClassName="w-5 h-5" />
              </div>
          ...
          <div className="space-y-8">
            ...
            <div>
              ...
              <div className="flex items-center gap-4">
                ...
                {/* Shipment Included badge removed */}
              </div>
                  ...
                  <SheetContent className="w-full sm:max-w-md overflow-y-auto">
                    <SheetHeader className="mb-6">
                      ...
                      <div className="text-center mt-2">
                        <span className="text-sm text-muted-foreground block">Estimated Price</span>
                        <span className="text-xl font-bold text-primary">₹{Math.round(pricing.total).toLocaleString()}</span>
                      </div>
                    </SheetHeader>
                    ...
                    <div className="space-y-8 pb-8">
                      ...
                      <div>
                        ...
                        <div className="flex gap-3">
                          {(["14K", "18K", "22K", "9K"] as const).map(kt => (
                            <button
                              key={kt}
                            ...
                            >
                              ...
                              <span className={cn(
                                ...
                                {product.inStock && kt === "22K" ? "Ready" : "Made to Order"}
                              </span>
                            </button>
                          ))}
                        </div>
                          ...
                          {(["Yellow", "White", "Rose"] as const).map(color => (
                            ...
                            >
                              ...
                              <span className={cn(
                                ...
                                {product.inStock && color === "Yellow" ? "Ready" : "Made to Order"}
                              </span>
                            </button>
                          ))}
                        </div>
                          ...
                          {(["EF,VVS", "EF,IF-VVS1"] as const).map(quality => (
                            ...
                            >
                              ...
                              <span className={cn(
                                ...
                                {product.inStock && quality === "EF,VVS" ? "Ready" : "Made to Order"}
                              </span>
                            </button>
                          ))}
                        </div>
                      ...
                      {((product.categoryName && product.categoryName.toLowerCase().includes("ring")) ||
                        (product.categoryName && product.categoryName.toLowerCase().includes("bangle")) ||
                        (product.categoryName && product.categoryName.toLowerCase().includes("bracelet"))) && (
                          <div>
                            <div className="flex items-center justify-between mb-3">
                              <h3 className="font-serif text-lg font-semibold text-primary">
                                {(product.categoryName && (product.categoryName.toLowerCase().includes("bangle") || product.categoryName.toLowerCase().includes("bracelet")))
                                  ? "Bangle Size"
                                  : "Ring Size"}
                              </h3>
                              <Dialog>
                      ...
                      <div className="space-y-4 pt-4 border-t border-border">
                        ...
                        <div className="space-y-3">
                          ...
                          <div>
                            ...
                            <textarea
                              className="w-full p-2 rounded-md border border-input bg-background resize-none h-20"
                              placeholder="Enter your delivery address"
                              value={customOrderForm.address}
                              onChange={(e) => setCustomOrderForm({ ...customOrderForm, address: e.target.value })}
                            />
                          </div>
                        </div>
                      </div>
                    ...
                    <SheetFooter>
                      <Button
                        ...
                        onClick={() => {
                          if (!customOrderForm.name || !customOrderForm.phone || !customOrderForm.address) {
                          ...
                          const message = `*New Custom Order Request*%0A%0A` +
                            `*Customer Details:*%0A` +
                            `Name: ${customOrderForm.name}%0A` +
                            `Phone: ${customOrderForm.phone}%0A` +
                            `Address: ${customOrderForm.address}%0A%0A` +
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-6">
              {/* Gold Details */}
              <div className="bg-card p-4 rounded-xl border border-border/50 shadow-sm hover:shadow-md transition-all duration-300 group">
                <div className="flex items-center gap-2 text-primary font-semibold border-b border-border/50 pb-2 mb-3 group-hover:text-accent transition-colors">
                  <Coins className="w-5 h-5" />
                  <span className="uppercase tracking-wide text-xs">Gold Details</span>
                </div>
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{pricing.netWeight ? pricing.netWeight.toFixed(3) : product.details.gold.netWt.toFixed(3)} g</span>
                  </div>
                  <div className="flex justify-between">
              ...
              <div className="bg-card p-4 rounded-xl border border-border/50 shadow-sm hover:shadow-md transition-all duration-300 group">
                <div className="flex items-center gap-2 text-primary font-semibold border-b border-border/50 pb-2 mb-3 group-hover:text-accent transition-colors">
                  ...
                  <span className="uppercase tracking-wide text-xs">Dimensions</span>
                </div>
                <div className="space-y-2 text-sm">
                  {product.details.gold.dimensions && Object.entries(product.details.gold.dimensions).map(([key, value]) => (
                    value && value !== "N/A" && (
                  ...
                  {(!product.details.gold.dimensions || Object.values(product.details.gold.dimensions).every(v => !v || v === "N/A")) && (
                    <div className="text-center text-muted-foreground italic py-2">No dimensions available</div>
                  )}
                  {selectedSize && (
                    <div className="flex justify-between border-t border-dashed border-border/40 pt-2 mt-2">
                      <span className="text-muted-foreground">Selected Size</span>
                      <span className="font-medium text-accent">{selectedSize}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-card p-4 rounded-xl border border-border/50 shadow-sm hover:shadow-md transition-all duration-300 group">
                <div className="flex items-center gap-2 text-primary font-semibold border-b border-border/50 pb-2 mb-3 group-hover:text-accent transition-colors">
                  ...
                  <span className="uppercase tracking-wide text-xs">Diamond Details</span>
                </div>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{product.details.diamond.count} pcs</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Count</span>
                    <span className="font-medium">{product.details.diamond.count} pcs</span>
              ...
              <div className="bg-card p-4 rounded-xl border border-border/50 shadow-sm hover:shadow-md transition-all duration-300 group">
                <div className="flex items-center gap-2 text-primary font-semibold border-b border-border/50 pb-2 mb-3 group-hover:text-accent transition-colors">
                  <Gem className="w-5 h-5" />
                  <span className="uppercase tracking-wide text-xs">Color Stones</span>
                </div>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Type</span>
                    <span className="font-medium">{selectedStoneQuality}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Total Weight</span>
                    <span className="font-medium">{product.details.colorStones.weight}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Count</span>
                    <span className="font-medium">{product.details.colorStones.count} pcs</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Accordion Sections for Details & Price */}
            <Accordion type="single" collapsible defaultValue="item-2" className="w-full border rounded-lg bg-card">

              {/* Product Details Accordion */}
              {/* The 4-Box Detail Grid was here */}
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-border/50">
                          <tr className="divide-x divide-border/50">
                            ...
                            <td className="px-4 py-2 text-muted-foreground">{pricing.netWeight ? pricing.netWeight.toFixed(3) : product.details.gold.netWt.toFixed(3)} g</td>
                            <td className="px-4 py-2 text-muted-foreground">₹{pricing.goldRate}/gm</td>
                            <td className="px-4 py-2 text-right font-medium">₹{Math.round(pricing.goldValue).toLocaleString()}</td>
                          </tr>
                          {pricing.diamondBreakup && pricing.diamondBreakup.length > 0 ? (
                            pricing.diamondBreakup.map((item: any, idx: number) => (
                              <tr key={`dia-${idx}`} className="divide-x divide-border/50">
                                <td className="px-4 py-2 font-medium">{item.name || 'Diamond'}</td>
                                <td className="px-4 py-2 text-muted-foreground">{item.weight} ct</td>
                                <td className="px-4 py-2 text-muted-foreground">₹{item.rate}/ct</td>
                                <td className="px-4 py-2 text-right font-medium">₹{Math.round(item.price).toLocaleString()}</td>
                              </tr>
                            ))
                          ) : (
                          ...
                          {pricing.stoneBreakup && pricing.stoneBreakup.length > 0 ? (
                            pricing.stoneBreakup.map((item: any, idx: number) => (
                              <tr key={`stone-${idx}`} className="divide-x divide-border/50">
                                <td className="px-4 py-2 font-medium">{item.name || 'Color Stone'}</td>
                                <td className="px-4 py-2 text-muted-foreground">{item.weight} ct</td>
                                <td className="px-4 py-2 text-muted-foreground">₹{item.rate}/ct</td>
                                <td className="px-4 py-2 text-right font-medium">₹{Math.round(item.price).toLocaleString()}</td>
                              </tr>
                          ...
                          {(pricing.certificationCost > 0 || true) && (
                            <tr className="divide-x divide-border/50">
                              <td className="px-4 py-2 font-medium">Certification Cost</td>
                              <td className="px-4 py-2 text-muted-foreground" colSpan={2}>-</td>
                              <td className="px-4 py-2 text-right font-medium">₹{Math.round(pricing.certificationCost).toLocaleString()}</td>
                            </tr>
                          )}
                          <tr className="divide-x divide-border/50">
            ...
            <p className="text-xs text-muted-foreground mt-3 text-center">
              Shipment charges will be calculated at checkout
            </p>

...
const Shop = () => {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    if (selectedCategory !== "All") {
      ...
      const catObj = allCategoriesData.find(c => c.name === selectedCategory);
      if (catObj) {
        if (product.category !== catObj._id) return false;
      } else {
        // Edge case: Category name doesn't exist in data? Should not happen unless mismatched.
        // If we can't find ID, maybe filter by name if backend changed? But currently IDs.
        return false;
      }
    }

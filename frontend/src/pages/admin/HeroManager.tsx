...
const HeroManager = () => {
    ...
    const fetchImages = async () => {
        try {
            ...
            const res = await fetch(`${API_BASE_URL}/api/hero/admin`, {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const url = editingImage
                ? `${API_BASE_URL}/api/hero/${editingImage._id}`
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await fetch(`${API_BASE_URL}/api/hero/${id}`, {
                method: 'DELETE',
                headers: { Authorization: `Bearer ${token}` }
            });
                    ...
                    <TableBody>
                        {images.map((img) => (
                            <TableRow key={img._id}>
                                <TableCell>
                                    <img src={`${API_BASE_URL}/${img.image}`} alt={img.title} className="w-16 h-10 object-cover rounded" />
                                </TableCell>
                                <TableCell>
